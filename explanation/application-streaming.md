...
The streaming stack includes additional controls that enhance the user experience during a streaming session. These controls are accessible via the stream bar and include:

- **Session Share Button**: Allows users to share the current session with others. When activated, it temporarily releases the keyboard from the stream handle and re-captures it upon closing.
- **Connect ADB Button**: Enables users to connect to the Android Debug Bridge (ADB) for the instance being streamed. This button indicates its active state when the ADB panel for the instance is open.
...
You can request AMS to create an instance and AMS will be able to dynamically create a session for the newly launched instance by contacting the stream agent. The agent mediates communication between the stream gateway and AMS and provides the necessary session information to the AMS. The AMS proceeds with successfully creating an instance that can be streamed.
